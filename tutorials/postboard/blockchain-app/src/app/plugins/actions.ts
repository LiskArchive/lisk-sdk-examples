 ...
 */
import { codec, db, validator as liskValidator } from 'lisk-sdk';
import {
	addressIndexPluginStore,
	CreateUserListRequest,
	createUserListRequest,
	DeleteUserListRequest,
	deleteUserListRequest,
	GetUserListByAddressRequest,
	GetUserListByTopicRequest,
	getUserListByTopicRequest,
	getUserListsByAddressRequest,
	topicIndexPluginStore,
} from './schema';

const { validator, LiskValidationError } = liskValidator;

const stringToBuffer = (prefix: string) => Buffer.from(prefix, 'utf-8');

const dbKey = (prefix: Buffer, key: Buffer) => Buffer.concat([prefix, key]).toString('binary');
...
export class UserTrackerActions {
	...
	public async createUserList(params?: Record<string, unknown>): Promise<unknown> {
		...
		for (const address of request.addresses) {
			...
			try {
				...
				await this._db.put(
					addressKey,
					codec.encode(addressIndexPluginStore, {
	...
	public async getUserListsByAddress(
