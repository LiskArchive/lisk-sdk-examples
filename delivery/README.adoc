...
Both sender and carrier need to sign a transaction, that verifies that the packet has been passed on to the carrier.
...
Both sender and carrier need to sign a transaction, that verifies that the packet has been passed on to the recipient.
...
* Signed by: Owner
* Required data: packetPublickey, receiverID, receiverLocation, senderLocation, porto, minSecurity, minTrust, estTravelTime
...
** `minSecurity` is the minimum amount, a person has to pay to register as a carrier for this packet.
...
* Signed by: Owner
* Required data: timestamp, packetID, security, carrierId
...
`security` is a certain amount of tokens, that the carrier must afford, to register as Carrier for this packet.
The Security is paid to the packet account and deducted from the Carrier.
