 ...
 * Signed by packet.
 */
class LightAlarmTransaction extends BaseTransaction { <0>
...
<0> Extend the new transaction type form the BaseTransaction interface.
...
<3> Cache the packet account. For the `LightAlarmTransaction`, we only need the packet account data.
