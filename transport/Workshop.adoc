...
* implement `applyAsset()` and `undoAsset()`
...
Next, you store the deducted number of tokens in a custom property in the `asset` field so we can later know how much tokens we had locked as a security.
...
NOTE: Undoing the locked tokens can be done by removing the custom property in the `asset` field and adding the number of tokens again to the account's `balance`.

==== Task: Complete the `undoAsset()` function
There is some code missing for the `undoAsset` function where we reset the carrier's balance.
Add the following snippet to the `const updatedCarrier` which sets the balance to the original balance.
...
Implement the FinishTransport-Tx
