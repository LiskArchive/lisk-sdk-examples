...
The packet itself will be an active participant in the transportation process, as it will track itself with sensors during the transportation.
This is to ensure, that it hasn't been manipulated.
...
For convenient usage of the system, a client application will be provided, that visualizes all relevant information about the supplychain tracking system, and that let's users interact with the system by posting custom transactions.
...
During this step, the sender defines
* `packetId`: The ID of the packet that has been initialized in the step before.
* `postage`: This amount will be transfered form the senders balance to the packet balance, when registering the packet on the network
...
With this transaction, the Carrier will lock the specified `security` of the packet in his account. This secuiry cannot be accessed by the carrier, unless the transport has been finished successfully.
...
* The packetID that the recipient received
* The status of the transport, which has 2 options: `"success"` or `"fail"`
...
Add the following features to the web application from <<_Part_1_track_a_packet_on_the_blockchain, Part 1: Track a packet on the blockchain>>
