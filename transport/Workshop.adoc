...
class LightAlarmTransaction extends BaseTransaction { <1>
    ...
    validateAsset() { <5>
        const errors = [];
        /* Write logic for validateAsset() here */
        return errors;
    }
...
Do do this, it needs to be checked, if the timestamp is present in the transaction ibject, and if it has the correct data type.
