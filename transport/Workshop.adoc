...
Try to implement the xref:transactions/register-packet.js[missing logic] for `undoAsset()` yourself by reverting the steps of the `applyAsset()` function.
...
You can find a more detailed explanation in the custom transactions deep dive article which you can find on [our blog](https://lisk.io/blog/tutorial/custom-transactions-statestore-basetransaction-transfer-transaction#6658). Look out for the section `B/ Combining Filters`.
...
Click here to see the xref:trasnactions/finish-transport.js[full code for FinishTransportTransaction]
...
Sometimes it might be needed to cache accounts or other data from the database, that a
...
xref:transactions/finish-transport.js[Write your own logic here], for the case of a successful transport of the packet.
...
=== 2.3 Test out the full process with the client app
