...
The only thing you need to implement in this step yourself is the `validateAsset` function.
...
<1> Require the custom trasnaction type.
...
const packetCredentials = { /* Insert the credentials of the packet here */ } <3>
...
setInterval(() => {                                                          <4>
	...
	if(state === 1) {
        ...
        tx.sign(packetCredentials.passphrase);                               <5>
...
<2> Enter here the IP of the node you want to reach for API requests
<3> Note: Always update to the package you are using
<4> Check the status of the sensor in a certain intervall (here: 1 second)
<5> Signed by package
