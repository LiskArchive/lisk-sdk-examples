...
│       ├── package.json
│       └── read_temperature_and_humidity.js
├── node <3>
│   ├── index.js
│   └── package.json
├── transactions <4>
...
class LightAlarmTransaction extends BaseTransaction {
    ...
    applyAsset(store) {} // insert implementation below

    undoAsset(store) {} // insert implementation below
}
module.exports = LightAlarmTransaction;
----

<1> Set the transaction type (this has to be unique in the network).
<2> Set the transaction fee. This needs to be paid by the sender when posting the transaction to the network.
Set it to `0` for now, so the packet doesn't need any funds to send an alarm transaction.
<3> Cache the packet account. To be able to work on a consistent data set, cache the needed data from the database.
For the `LightAlarmTransaction`, we only need the packet account data.
<4> Static checks for presence and correct datatype of `timestamp`, which holds the timestamp of when the alarm was triggered.

.Implementation of applyAsset() and undoAsset()
[source,js]
----
applyAsset(store) {
    const errors = [];
...
Once we have created the new custom transaction type `LightAlarmTransaction`, it needs to be registered with the node application.
