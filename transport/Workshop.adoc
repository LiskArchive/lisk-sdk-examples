...
<1> The `LightAlarmTransactin`is extended from the `BaseTransaction` interface.
...
Copy the snippet and replace the àpplyAsset` function in `light-alarm.js` with it, to complete the implementation of the transaction type.
...
<5> Insert the below code snippet of the tracking script.
...
const packetCredentials = { /* Insert the credentials of the packet here */ }
