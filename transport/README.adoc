...
Before registering the packet on the network, the owner needs to create a new account for the packet and store the passphrase on the IoT device inside the packet.
...
This transaction should only go off, when the packet is traveling (between `startDeliveryTransaction` and `finishDeliveryTransaction`).

registerPacketTransaction::
* Signed by: Owner
* Required data: packetPublickey, receiverID, receiverLocation, senderLocation, postage, security, minTrust, estTravelTime
...
** `postage` needs to be paid by the owner, and is stored in the account of the packet.
...
If the carrier behaves maliciously, the security will not be unlocked (it will be transferred to the packet owners account, together with the postage).
...
*** Security tokens and Postage will be transferred to packet Owner
