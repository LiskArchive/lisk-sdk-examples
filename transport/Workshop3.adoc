...
At before we can start the transport of the packet, we first need to transfer some tokens to the carrier account, which doesn't have any tokens initially.
This is necessary, because the carrier needs to lock the `security` in the carriers account.

To do this, go to the http://localhost:3000/faucet[Faucet page] and enter the carrier address and the amount of tokens that shall e transferred to this account.

Just make sure, it's enough so the carrier can afford to lock the `security` of the packet,
